...
!-------------------- namelist -----------------------------------------
   ...
   !! negative value == days... noted below
...
!     ----- compute coefficients -----
      ...
      !! ...86400 seconds == 1 day... so specifing NEGATIVE KA seems to be
      !! way to determing cooling in terms of DAYS; conversion done below

      if (ka < 0.) ka = -86400.*ka
      if (ks < 0.) ks = -86400.*ks
      if (kf < 0.) kf = -86400.*kf
      
      !! here get tka==1/tau, so tau==ka; specify it this way because user might
      !! enter ka=0, which results down the line in dT/dt=0 i.e. NO NEWTONIAN
      !! COOLING (my goal), as opposed to INSTANTANEOUS cooling
      !! ...
      !! so a bit convoluted, but setting ka=0 will "turn off" cooling
...
!-----------------------------------------------------------------------
      ...
      !! difference between boundary layer damping and atmoshpere damping
      !! ...is usually negative, but if both zero, positive... seems to apply
...
!  ----- compute damping -----
    !! sigma_b is threshold level for stratosphere; above it, non-constant
    !! damping, function of latitude and 'tcoeff', a scaling prop to height
    !! above sigma_b that ADDS to the default (1/tau)==tka
    !! ...
    !! remember sigma goes from 0 at surface to 1 at 'top of atmosphere'
         sigma(:,:) = p_full(:,:,k)*rps(:,:)
