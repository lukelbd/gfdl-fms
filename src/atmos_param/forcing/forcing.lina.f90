...
! This forcing also allows the Lindren et al 2018 (doi: 10.1029/2018JD028537) forcing 
! to add forced planetary scale waves (set lsp = .true. in namelist file). [Lina Boljka]
...
real :: lat_trop_ref = 0, m_lambda = 1., q_0_heating = 6.
real :: pt_lsp = 20000., p0_lsp = 80000.
...
namelist /forcing_nml/  no_forcing, rdamp_decomp, & !ndamp_decomp, &
  ...
  trflux, trsink, &
  conserve_energy, lsp, m_lambda, q_0_heating,pt_lsp, p0_lsp
!print*,"namelist_from_input"
!-----------------------------------------------------------------------
...
integer :: id_teq, id_ndamp_mean, id_ndamp_anom, id_rdamp_mean, id_rdamp_anom, &
  ...
  id_tdt_diss, id_heat_diss, id_udt_spg, id_vdt_spg, id_tdt, id_udt, id_vdt
...
! the current number of arguments is the limit; the function will not provide more...
!-----------------------------------------------------------------------
...
real, intent(in),    dimension(:,:,:)              :: um, vm, tm
...
!real, dimension(size(t,1)) :: deg_lon 
real, dimension(size(t,1)) :: deg_lon_out 
real, dimension(size(t,1),size(t,2)) ::rad_lon_2d

! manual
!do i=1,size(t,1)
!  deg_lon(i) = i*2.*pi/size(t,1)
!enddo
!do j = 1, size(t,2)
!  rad_lon_2d (:,j) = deg_lon(:)
!enddo

! call deg_lon function - latitude is inputted via rad_lat_2d via get_deg_lon; 
call get_deg_lon(deg_lon_out)
do i=is,ie
  rad_lon_2d(i,:) = deg_lon_out(i)*pi/180.
enddo
...
call newtonian_damping_da ( Time, lat,rad_lon_2d, ps, p_full, t, ttnd, tdamp, teq, mask )
...
if (id_udt   > 0) used = send_data ( id_udt,   udt,  Time, is, js)
if (id_vdt   > 0) used = send_data ( id_vdt,   vdt,  Time, is, js)
...
!#######################################################################
 ...
 subroutine newtonian_damping_da ( Time, lat, lon, ps, p_full, t, tdt, tdamp, teq, mask )
 !subroutine newtonian_damping_da ( Time, lat, ps, p_full, t, tdt, tdamp, teq, mask ) !call newtonian_damping_da ( Time, lat,rad_lon_2d, ps, p_full, t, ttnd, tdamp, teq, mask )
...
real, dimension(size(t,1),size(t,2),size(t,3)) :: &
  t_pk, t_hs, t_pv, t_std, dTeq ! helper containers
...
real    :: phi0_lsp, sigma_phi_lsp ! additional constants needed for localised heating of Lindgren et al 2018 (lsp)
!logical :: lsp 
!real :: m_lambda, q_0_heating, p0_lsp

!print*,"inside_newtonian_damping"
!-----------------------------------------------------------------------
! Constants
...
!lsp = .true. 
!m_lambda = 1.
!q_0_heating = 150. ! use this unrealistic number for tests and for Teq changes rather than heating changes (tdt)
!q_0_heating    = q_0_heating/seconds_per_day ! for lsp: need heating rate in seconds not days! number given in days originally. 
!q_0_heating    = q_0_heating*600 ! multiply by 600s - to get per timestep (maybe this will help giving me what I want...) - too big
!p0_lsp         = 80000. ! for lsp - level at which localised heating starts
!pt_lsp         = 20000. ! for lsp - level at which localised heating stops (now defined in namelist)
phi0_lsp       = 45. * pi/180. ! for lsp - latitude at which localised heating is centred
sigma_phi_lsp  = 0.175 ! for lsp - presumably related to the width of heating in latitude (standard deviation in Gaussian distribution)
...
do k=1,size(t,3)
  ...
  ! at first I used lsp on Equillibrium temperature - but really should be using it on heating rate - i.e. tdt
  if (lsp) then
    ! add Lindgren,Sheshadri,Plumb 2018 localised heating perturbation to excite planetary waves (if lsp=.true.)
    ! doi: 10.1029/2018JD028537 ; multiply by 40 since this is typical damping rate in free atmosphere
    where ((p_full(:,:,k) <= p0_lsp) .and. (p_full(:,:,k) >= pt_lsp))
        dTeq (:,:,k) = q_0_heating * sin(m_lambda*lon(:,:))*exp(-0.5*((lat(:,:)-phi0_lsp)/sigma_phi_lsp)**2)*sin(pi*log(p_full(:,:,k)/p0_lsp)/log(pt_lsp/p0_lsp)) 
    elsewhere
        dTeq (:,:,k) = 0.   
    endwhere 
    !do i = 1,size(t,1)
    !    do j = 1,size(t,2)
     !        if ((p_full(i,j,k) <= p0_lsp) .and. (p_full(i,j,k) >= pt_lsp)) then
     !            dTeq (i,j,k) = q_0_heating * sin(m_lambda*lon(i,j))*exp(-0.5*((lat(i,j)-phi0_lsp)/sigma_phi_lsp)**2)*sin(pi*log(p_full(i,j,k)/p0_lsp)/log(pt_lsp/p0_lsp))  
     !        else
     !            dTeq (i,j,k) = 0
     !        endif
     !   enddo
  ...
  do l=1,2
    ...
    endwhere
    !if (trim(strat_mode) == 'hs') then
    !if (trim(strat_mode) == 'hs' .or. trim(strat_mode) == 'pk') then
      !     ----- Held Suarez or Polvani Kushner -----
      ! Damping rate
      !where (s(:,:) > sigma_b)
      !  tdamp(:,:,k,l) = tktrop(l) + tksurf_decomp(:,:,l)*(s(:,:)-sigma_b)/(1.0-sigma_b)
      !elsewhere
    ...
    !else if (trim(strat_mode) == 'pk') then
      !     ----- Polvani Kushner [donno why this is here so I removed it; this is modified PK2002!] -----
      ! Damping rate
    ...
    !else if (trim(strat_mode) == 'da') then
      !     ----- Davis et al. - Also this is some modified damping stuff I don't want! -----
    ...
    !if (lsp) then
    ! add Lindgren,Sheshadri,Plumb 2018 localised heating perturbation to excite planetary waves (if lsp=.true.)
    ! doi: 10.1029/2018JD028537 ; [here it is divided by tdamp as it is later on multiplied by it!] BAD!!! DON'T USE!
    !  where (p_full(:,:,k) .le. p0_lsp .and. p_full(:,:,k) .ge. pt_lsp)
    !    teq(:,:,k) = teq(:,:,k) + (q_0_heating/(tdamp(:,:,k,l)))* sin(m_lambda*lon(:,:))*exp(-0.5*((lat(:,:)-phi0_lsp)/sigma_phi_lsp)**2)*sin(pi*log(p_full(:,:,k)/p0_lsp)/log(pt_lsp/p0_lsp)) 
    !  endwhere    
    !end if
    ...
    !  tdt(:,:,k,2) = -tdamp(:,:,k,2)*t_decomp(:,:,2)
      ...
      !  where (p_full(:,:,k) .le. p0_lsp .and. p_full(:,:,k) .ge. pt_lsp)
      !    tdt(:,:,k,2) = tdt(:,:,k,2) + q_0_heating * sin(m_lambda*lon(:,:))*exp(-0.5*((lat(:,:)-phi0_lsp)/sigma_phi_lsp)**2)*sin(pi*log(p_full(:,:,k)/p0_lsp)/log(pt_lsp/p0_lsp)) 
      !  endwhere    
      !end if
    !endif
  enddo
  !tdt(:,:,k,1)   = -tdamp(:,:,k,1)*(t(:,:,k) - (teq(:,:,k) +(dTeq(:,:,k)/(seconds_per_day*tdamp(:,:,k,1) ) ) ) ) ! this seems to work better. - crashes after 1000 days with q0=6K
  tdt(:,:,k,1)   = -  (tdamp(:,:,k,1)*t(:,:,k)) + (tdamp(:,:,k,1)* teq(:,:,k)) + (dTeq(:,:,k)/seconds_per_day)  ! let's see if it works... - this makes it too weak...
  !tdt(:,:,k,1)   = -  tdamp(:,:,k,1)*(t(:,:,k) - teq(:,:,k)) + (dTeq(:,:,k)/seconds_per_day) ! original forcing
  tdt(:,:,k,2)   = 0.0
  tdamp(:,:,k,2) = 0.0
  !if (lsp) then
    ! add Lindgren,Sheshadri,Plumb 2018 localised heating perturbation to excite planetary waves (if lsp=.true.)
    ! doi: 10.1029/2018JD028537 ; For some reason this below is not working properly... (or maybe it didn't in previous setup)
  !  where (p_full(:,:,k) .le. p0_lsp .and. p_full(:,:,k) .ge. pt_lsp)
  !    tdt(:,:,k,1) = tdt(:,:,k,1) + q_0_heating * sin(m_lambda*lon(:,:))*exp(-0.5*((lat(:,:)-phi0_lsp)/sigma_phi_lsp)**2)*sin(pi*log(p_full(:,:,k)/p0_lsp)/log(pt_lsp/p0_lsp)) 
  !  endwhere    
  !end if
enddo
...
subroutine rayleigh_damping ( ps, p_full, u, v, udt, vdt, uvdamp, mask ) !call rayleigh_damping ( ps, p_full, u, v, utnd, vtnd, uvdamp, mask=mask )
