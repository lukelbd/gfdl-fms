...
!----- write namelist to logfile -----
   ...
   unit = stdlog()
   if ( mpp_pe() == mpp_root_pe() ) write (iunit, nml=main_nml)
...
!  print number of tracers to logfile
   if (mpp_pe() == mpp_root_pe()) then
        iunit = stdlog()
        write (iunit, '(a,i3)') 'Number of tracers =', ntrace
        write (iunit, '(a,i3)') 'Number of prognostic tracers =', ntprog
        write (iunit, '(a,i3)') 'Number of diagnostic tracers =', ntdiag
   endif
