...
use              fms_mod, only: mpp_pe, mpp_root_pe, write_version_number, read_data, file_exist
...
if (file_exist(trim(file))) then
  if (mpp_pe() == mpp_root_pe()) write(*,*) 'Reading mean_state.nc'
  call read_data(trim(file), 'ug',   ug(:,:,:), grid_domain)
  call read_data(trim(file), 'vg',   vg(:,:,:), grid_domain)
  call read_data(trim(file), 'tg',   tg(:,:,:), grid_domain)
  call read_data(trim(file), 'psg', psg(:,:  ), grid_domain)
  vorg   = 0.
  divg   = 0.
  ln_psg = log(psg)
else
