...
public :: spectral_init_cond, get_topography
...
subroutine spectral_init_cond(reference_sea_level_press, triang_trunc, use_virtual_temperature, topography_option, &
                              ...
                              p_press, p_sigma, exponent, ocean_topog_smoothing, surf_geo_factor, pk, bk, &
                              vors, divs, ts, ln_ps, ug, vg, tg, psg, vorg, divg, surf_geopotential, ocean_mask)
...
call spectral_initialize_fields(reference_sea_level_press, triang_trunc, choice_of_init, initial_temperature, &
                  surf_geopotential(:,:)*surf_geo_factor, ln_ps, vors, divs, ts, psg, ug, vg, tg, vorg, divg)
...
else if(trim(topography_option) == 'input' .OR. trim(topography_option) == 'pulse' .OR. &
  trim(topography_option) == 'ramp') then
