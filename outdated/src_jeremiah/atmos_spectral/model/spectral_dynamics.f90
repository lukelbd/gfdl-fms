...
use          constants_mod, only: rdgas, rvgas, grav, cp_air, omega, radius, pi, seconds_per_day
...
use spectral_init_cond_mod, only: spectral_init_cond, get_topography
...
integer :: id_ps, id_u, id_v, id_t, id_vor, id_div, id_omega, id_wspd, id_slp, id_zsurf
...
real,    allocatable, dimension(:)   :: g_grid
real,    allocatable, dimension(:,:) :: vt_grid
...
real            :: tfact, tmod, tmodold, pulsestep
real, public    :: surf_geo_factor = 1.0
real            :: days_per_second = 1.0/seconds_per_day
...
namelist /spectral_dynamics_nml/ use_virtual_temperature, damping_option,                            &
                                 ...
                                 print_interval, num_steps, geop_pulse_init, geop_pulse_dur,         &
                                 read_topography
...
subroutine spectral_dynamics_init(Time, Time_step_in, tracer_attributes, dry_model_out, &
             nhum_out, ocean_mask)
...
call get_time(Time, surf_sec, surf_day)
if(trim(topography_option) == 'pulse') then
  tfact   = dble(surf_day) + dble(surf_sec)*days_per_second
  if (tfact .LT. geop_pulse_init) then
    surf_geo_factor = 0.0
  else if (tfact .GE. geop_pulse_init .AND. tfact .LT. geop_pulse_init+geop_pulse_dur) then      ! For single pulses
    pulsestep = tfact - geop_pulse_init
    surf_geo_factor = sin((pulsestep)/(geop_pulse_dur)*pi)!**2
  else
    surf_geo_factor = 0.0
  endif
  tmodold = tmod
else if (trim(topography_option) == 'ramp') then
  tfact   = dble(surf_day) + dble(surf_sec)*days_per_second
  if (tfact .LE. geop_pulse_init) then
    surf_geo_factor = 0.0
  else if (tfact .LT. geop_pulse_init+geop_pulse_dur) then
    tmod            = mod((tfact-geop_pulse_init),geop_pulse_dur)
    surf_geo_factor = tmod/geop_pulse_dur
  else
    surf_geo_factor = 1.0
  endif
else
...
!if (mpp_pe() .EQ. mpp_root_pe()) write(*,*) surf_geo_factor, 'init'
...
if(file_exist(trim(file))) then
  ...
  call spectral_init_cond(reference_sea_level_press, triang_trunc, use_virtual_temperature, topography_option,  &
                          ...
                          exponent, ocean_topog_smoothing, surf_geo_factor, pk, bk,                             &
...
allocate (vt_grid(is:ie, js:je))
allocate (g_grid(js:je))
...
if(dry_model) then
  ...
  call compute_geopotential(tg(:,:,:,current), ln_p_half, ln_p_full, phig_full, phig_half, &
      surf_geo_factor, grid_tracers(:,:,:,current,nhum))
endif
...
if(minval(tg(:,:,:,future)) < valid_range_t(1) .or. maxval(tg(:,:,:,future)) > valid_range_t(2)) then
  write(*,*) minval(tg(:,:,:,future)),maxval(tg(:,:,:,future))
...
! Update surf_geo_factor
if (trim(topography_option) == 'pulse') then 
  tfact   = dble(days) + dble(seconds)*days_per_second
  if (tfact .LT. geop_pulse_init) then
    surf_geo_factor = 0.0
  else if (tfact .GE. geop_pulse_init .AND. tfact .LT. geop_pulse_init+geop_pulse_dur) then       ! For single pulses
    pulsestep = tfact - geop_pulse_init
...
!if (mpp_pe() .EQ. mpp_root_pe()) write(*,*) surf_geo_factor, 'run'
...
!call write_data(trim(file), 'surf_geo_factor', surf_geo_factor, no_domain=.true.)
do nt=1,num_time_levels
...
id_zsurf = register_diag_field(mod_name, 'zsurf', (/id_lon,id_lat/), Time, 'geopotential height at the surface', 'm')

if(id_pk    > 0) used = send_data(id_pk, pk, Time)
if(id_bk    > 0) used = send_data(id_bk, bk, Time)
!if(id_zsurf > 0) used = send_data(id_zsurf, surf_geopotential*surf_geo_factor/grav, Time)
...
if(id_zfull > 0 .or. id_zhalf > 0) then
  call compute_pressures_and_heights(t_grid, p_surf, z_full, z_half, p_full, p_half, surf_geo_factor)
...
20    continue
      ...
      slp(i,j) = p_surf(i,j)*((t_low(i,j) + gamma*surf_geopotential(i,j)*surf_geo_factor/grav)/t_low(i,j))**expf_inverse
    enddo
  enddo
...
!===================================================================================
real function total(array)
  real, intent(in), dimension(:) :: array
  integer                        :: arrsize, i
  
  arrsize = size(array,1)
  do i = 1, arrsize
    total = total + array(i)
  enddo
  RETURN
