...
!-----------------------------------------------------------------------
   ...
   real, dimension(:,:,:), allocatable :: tstd
...
!     thermal forcing for held & suarez (1994) benchmark calculation
      ...
      call newtonian_damping ( Time, lat, ps, p_full, t, ttnd, teq, tdamp, mask )
...
!      if (id_tstd > 0) used = send_data ( id_tstd, tstd,  Time, is, js)
      ...
      if (id_tdamp > 0) used = send_data ( id_tdamp, tdamp, Time, is, js)
...
!     ----- calculate US standard atmos temperatures -----

      if (.not.allocated(ttmp)) allocate(ttmp(size(p_full,1),size(p_full,2)))
      if (.not.allocated(tstd)) allocate(tstd(size(p_full,1),size(p_full,2),size(p_full,3)))
...
!     ----- register diagnostic fields -----
      ...
      id_tdamp = register_diag_field ( mod_name, 'tdamp', axes(1:3), Time, &
                      'radiative damping', '1/sec'   , &
...
!#######################################################################
 ...
 subroutine newtonian_damping ( Time, lat, ps, p_full, t, tdt, teq, tdamp, mask )
...
!-----------------------------------------------------------------------
     ...
     sig_trop, t_pt, logpz, ttmp
       ...
       real, dimension(size(t,1),size(t,2),size(t,3)) :: & !tdamp, &
     sigma, t_stra, ttrop, t_trop, t_pv
...
!  ----- compute damping -----
        if (trim(damp) == 'linear') then
         ...
         endwhere
        else if (trim(damp) == 'hm') then
          logpz(:,:)   = -7.0*alog(sigma(:,:,k))
          where (sigma(:,:,k) <= 1.0 .AND. sigma(:,:,k) > sigma_b)
            tfactr(:,:)  = tcoeff*(sigma(:,:,k)-sigma_b)
            tdamp(:,:,k) = tka + cos_lat_4(:,:)*tfactr(:,:)
          elsewhere
            tdamp(:,:,k) = (1.5 + 1.21*tanh((logpz(:,:)-35.0)/7.0))*1.E-6
          endwhere
        else
