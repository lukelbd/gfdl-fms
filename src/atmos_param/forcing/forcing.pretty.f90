...
10    call close_file(unit)
    ...
    ! Radiative forcing
    id_forcing = register_diag_field( &
                 mod_name, 'forcing', axes(1:3), Time, &
                 'radiative forcing', 'deg_K/sec', &
                 missing_value=missing_value)

    ! Equilibrium temp
    id_teq = register_diag_field( &
             mod_name, 'teq', axes(1:3), Time, &
             'equilibrium temperature', 'deg_K', &
             missing_value=missing_value, range=(/0., 500./))

    ! Rate of temp change due to damping and damping rate
    id_tdt = register_diag_field( &
             mod_name, 'tdt', axes(1:3), Time, &
             'heating', 'deg_K/sec', &
             missing_value=missing_value)

    if (locked_heating) then
      id_heating = register_diag_field( &
                   mod_name, 'heating', axes(1:3), Time, &
                   'locked heating', 'deg_K/sec', &
                   missing_value=missing_value)
    endif

    id_ndamp = register_diag_field( &
               mod_name, 'ndamp', axes(1:3), Time, &
               'thermal damping coefficient', 'sec-1', &
               missing_value=missing_value)

    if (ndamp_decomp) then
      id_tdt_mean = register_diag_field( &
                    mod_name, 'tdt_mean', axes(1:3), Time, &
                    'heating, mean component', 'deg_K/sec', &
                    missing_value=missing_value)

      id_tdt_anom = register_diag_field( &
                    mod_name, 'tdt_anom', axes(1:3), Time, &
                    'heating, anomaly component', 'deg_K/sec', &
                    missing_value=missing_value)

      id_ndamp_mean = register_diag_field( &
                      mod_name, 'ndamp_mean', axes(1:3), Time, &
                      'thermal damping coefficient, mean component', 'sec-1', &
                      missing_value=missing_value)

      id_ndamp_anom = register_diag_field( &
                      mod_name, 'ndamp_anom', axes(1:3), Time, &
    ...
    if (conserve_energy) then
      id_tdt_diss = register_diag_field( &
                    mod_name, 'tdt_diss_rdamp', axes(1:3), &
                    Time, 'dissipative heating from frictional damping', 'deg_K/sec', &
                    missing_value=missing_value)

      id_heat_diss = register_diag_field( &
                     mod_name, 'diss_rdamp', axes(1:2), &
                     Time, 'integrated dissipative heating for frictional damping', 'W/m2')
    endif

    ! Rate of wind change due to friction
    id_udt = register_diag_field( &
             mod_name, 'udt', axes(1:3), Time, &
             'frictional zonal wind damping', 'm/s2', &
             missing_value=missing_value)

    id_vdt = register_diag_field( &
             mod_name, 'vdt', axes(1:3), Time, &
             'frictional meridional wind damping', 'm/s2', &
             missing_value=missing_value)

    id_rdamp = register_diag_field( &
               mod_name, 'rdamp', axes(1:3), Time, &
               'frictional damping coefficient', 'sec-1', &
               missing_value=missing_value)

    if (rdamp_decomp) then
      id_udt_mean = register_diag_field( &
                    mod_name, 'udt_mean', axes(1:3), Time, &
                    'frictional zonal wind damping, mean component', 'm/s2', &
                    missing_value=missing_value)

      id_udt_anom = register_diag_field( &
                    mod_name, 'udt_anom', axes(1:3), Time, &
                    'frictional zonal wind damping, anomaly component', 'm/s2', &
                    missing_value=missing_value)

      id_vdt_mean = register_diag_field( &
                    mod_name, 'vdt_mean', axes(1:3), Time, &
                    'frictional meridional wind damping, mean component', 'm/s2', &
                    missing_value=missing_value)

      id_vdt_anom = register_diag_field( &
                    mod_name, 'vdt_anom', axes(1:3), Time, &
                    'frictional meridional wind damping, anomaly component', 'm/s2', &
                    missing_value=missing_value)

      id_rdamp_mean = register_diag_field( &
                      mod_name, 'rdamp_mean', axes(1:3), Time, &
                      'frictional damping coefficient, mean component', 'sec-1', &
                      missing_value=missing_value)

      id_rdamp_anom = register_diag_field( &
                      mod_name, 'rdamp_anom', axes(1:3), Time, &
    ...
    if (strat_sponge) then
      id_udt_sponge = register_diag_field( &
                      mod_name, 'udt_sponge', axes(1:3), Time, &
                      'sponge zonal wind damping', 'm/s2', &
                      missing_value=missing_value)

      id_vdt_sponge = register_diag_field( &
                      mod_name, 'vdt_sponge', axes(1:3), Time, &
                      'sponge meridional wind damping', 'm/s2', &
                      missing_value=missing_value)

      id_sdamp = register_diag_field( &
                 mod_name, 'sdamp', axes(1:3), Time, &
